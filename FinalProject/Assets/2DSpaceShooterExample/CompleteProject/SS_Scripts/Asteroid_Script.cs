...
public class Asteroid_Script : MonoBehaviour 
{
	...
	void OnTriggerEnter2D(Collider2D other)
	{
		...
		{
            GameObject laserhit = Instantiate(LaserGreenHit, transform.position, transform.rotation);		//Instantiate LaserGreenHit 
			...
			if(health <= 0)
			{
                GameObject explor = Instantiate(Explosion, transform.position, transform.rotation);		//Instantiate Explosion
